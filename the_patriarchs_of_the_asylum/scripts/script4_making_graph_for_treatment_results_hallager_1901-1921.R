##### TREATMENT RESULTS AT JYSKE ASYL UNDER F.K. HALLAGER, 1901-1921
...
### DEVELOPMENT OF TREATMENT RESULTS UNDER F.K. HALLAGER, 1901-1921, LINE-GRAPH
# Visualizing the development with a line-graph

## FILTERING AND ADDING PERCENTAGE COLLUMS
# Filtering the years that F.K. Hallager acted as chief physician 
...
pct_hallager_treatment_results <- jyske_asyl_treatment_results %>% 
...
plot_hallager_treatment_results <- pct_hallager_treatment_results %>% 
  ggplot()+
  ...
  theme_bw()+
  labs(title="Development of treatment results - F.K. Hallager, 1901-1921", 
       subtitle= "Fig.6",
       x="Year", 
  ...
  scale_x_continuous( breaks = seq(1901, 1921, by = 2), 
                      minor_breaks = seq(1901, 1921, by = 1))+
...
# The finale graph:
plot_hallager_treatment_results

# Saving the graph as a png
ggsave("fig6_hallager_development_treatment_results_1901-1921.png", 
