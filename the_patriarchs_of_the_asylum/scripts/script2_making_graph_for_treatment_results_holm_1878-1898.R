##### TREATMENT RESULTS AT JYSKE ASYL UNDER R.A. HOLM, 1878-1898
...
### DEVELOPMENT OF TREATMENT RESULTS UNDER R.A. HOLM, 1878-1898, LINE-GRAPH
# Visualizing the development with a line-graph

## FILTERING AND ADDING PERCENTAGE COLLUMS
# Filtering the years that R.A. Holm acted as chief physician 
...
pct_holm_treatment_results <- jyske_asyl_treatment_results %>% 
  filter(year<1899) %>% 
...
plot_holm_treatment_results <- pct_holm_treatment_results %>% 
  ggplot()+
  ...
  theme_bw()+
  labs(title="Development of treatment results - R.A. Holm, 1878-1898", 
       subtitle= "Fig.4",
       x="Year", 
  ...
  scale_x_continuous( breaks = seq(1878, 1898, by = 2), 
                      minor_breaks = seq(1878, 1898, by = 1))+
...
# The finale graph:
plot_holm_treatment_results

# Saving the graph as a png
ggsave("fig4_holm_development_treatment_results_1878-1898.png", 
